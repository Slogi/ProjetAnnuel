...
class DartyPipeline(object):
    ...
    def save(self, row):
        ...
        if nb_result > 0:
            ...
            price_select = ("SELECT price_price_exponent, price_price_fraction FROM price WHERE price_ean = %(ean)s AND price_id_site = '02' ORDER BY price_date DESC LIMIT 1")
            ...
            if nb_result > 0:
                for r in rows:
                    ...
                    if (r[0] != row['price_exponent']) or (r[1] != row['price_fraction']):
                        ...
                        price_query = ("INSERT INTO price (price_ean, price_id_site, price_date, price_price_exponent, price_price_fraction) VALUES (%(ean)s, '02', %(date)s, %(price_exponent)s, %(price_fraction)s     )")
            ...
            link_select = ("SELECT link_ean FROM link WHERE link_id_site = '02' AND link_ean = %(ean)s")
            ...
            if nb_result > 0:
                ...
                link_query = ("INSERT INTO link (link_ean, link_id_site, link_url) VALUES (%(ean)s, '02', %(url)s)")
